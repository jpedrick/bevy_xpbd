...
fn transform_parent(mut parent: Query<&mut Transform, With<ParentTransform>>, time: Res<Time>) {
    if let Ok(mut parent_transform) = parent.get_single_mut() {
        parent_transform.rotate_around(Vec3::Z, Quat::from_rotation_z(0.001 * time.delta_seconds()));
...
fn render_rays(mut rays: Query<(&mut RayCaster, &mut RayHits)>, mut gizmos: Gizmos, sq: SpatialQuery) {
            ...
            while some_ricochet.is_some() && n_hits < 15 {
                if let Some(ricochet) = some_ricochet {
                    ...
                    if let Some(this_hit) = some_ricochet{
                        ...
                        let new_hit_location = last_hit_location + (ricochet.normal) * this_hit.time_of_impact as f32;
